...
def main(n_iters, n_warmup, inital_user_ix, n_users):
    ...
    samples_gamma = pd.read_csv(data_dir + 'fr_samples_gamma.csv')
    samples_mu_beta = pd.read_csv(data_dir + 'fr_samples_mu_beta.csv')
    samples_sigma_beta = pd.read_csv(data_dir + 'fr_samples_sigma_beta.csv')

    fin = data_dir + "adj-matrix-FR.csv"
    ...
    pd.DataFrame(beta_avg, index=index).to_csv('fr_samples_beta_stage2.csv')
    pd.DataFrame(theta_avg, index=index).to_csv('fr_samples_theta_stage2.csv')
